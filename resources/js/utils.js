...
function relationshipCalculator(person1, person2, data) {
    const langArray = getLang();
    const details = data["details"];
    // Finds the least common ancestor
    function leastCommonAncestor(p1, p2) {
...
function setSearchEvents(searchText, searchList, data, view, link = true) {
    ...
    searchText.addEventListener("focus", function (event) {
        event.currentTarget.setSelectionRange(0, event.currentTarget.value.length);
    });
    // On enter, try searching
    searchText.addEventListener("keydown", function (keyEvent) {
    ...
    searchText.addEventListener("input", function (event) {
        ...
        if (searchText.value.length < 3)
            return; // Don't start searching unless 3+ characters
